...
public class AVLTree{
    ...
    */
   private TreeNode restructure(TreeNode x, TreeNode y, TreeNode z){
       TreeNode a, b, c;
        TreeNode T0, T1, T2, T3;
        
        boolean zIsLeftChild = (z == y.leftChild);
        boolean yIsLeftChild = (y == x.leftChild);
        
        if(zIsLeftChild && yIsLeftChild){ //left left
            a = z;
            b = y;
            c = x;
            T0 = a.leftChild;
    ...
    public TreeNode insert(String s)
    {
        ...
        p = findInsertionPoint(s);
        newnode.height = 1;
        newnode.parent = p;
        
        if(s.compareTo(p.nodeValue) < 0){
            p.leftChild = newnode;
        }else{
        ...
        while(x != null){
            int leftChildHeight = (x.leftChild != null)?1 + x.leftChild.height:0;
            int rightChildHeight = (x.rightChild != null)? 1 + x.rightChild.height:0;
    ...
    public TreeNode findInsertionPoint(String s){
        TreeNode curr;
        TreeNode prev;
        curr = root;
        prev = root;
        ...
        while(curr != null){
            if(s.compareTo(curr.nodeValue) < 0){
                prev = curr;
                curr = curr.leftChild;
            }else if(s.compareTo(curr.nodeValue) > 0){
                prev = curr;
    ...
    public TreeNode finddeletePoint(String s){
        TreeNode curr;
        TreeNode prev;
        curr = root;
        prev = root;
    ...
    public TreeNode rotation(TreeNode currentnode,String s)
    {

        currentnode.recomputeHeight();
        int leftHeight=getSubTreeSize(currentnode.leftChild);
        int rightHeight=getSubTreeSize(currentnode.rightChild);
        if(leftHeight-rightHeight==2){
            if(getSubTreeSize(currentnode.leftChild)>getSubTreeSize(currentnode.rightChild)){
        ...
        if(rightHeight-leftHeight==2){
            if(getSubTreeSize(currentnode.rightChild)>getSubTreeSize(currentnode.leftChild)){
    ...
    public TreeNode leftRotate(TreeNode n2){
        TreeNode n1=n2.leftChild;
        n2.leftChild=n1.rightChild;
        n1.rightChild=n2;
        return n2;
    }
    public TreeNode doubleWithLeftChild (TreeNode n3){
        n3.leftChild = rightRotate(n3.rightChild);
        return leftRotate(n3);
    }
    public TreeNode rightRotate(TreeNode n1)
    {
        TreeNode n2=n1.rightChild;
        n1.rightChild=n2.leftChild;
        n2.leftChild=n1;
        return n2;
    }
    public  TreeNode doubleWithRightChild (TreeNode n1){
        n1.rightChild = leftRotate(n1.rightChild);
	...
	*/
	...
	public void remove(TreeNode node){
        ...
        if(s==(root.nodeValue)){    //remove node
            if(root.leftChild==null&&root.rightChild==null){
                root=null;
            }
        }
        deleteNode=findInsertionPoint(s);
